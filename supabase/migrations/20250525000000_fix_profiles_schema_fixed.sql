...
-- Run this in your Supabase SQL Editor (CORRECTED VERSION)
...
-- Add constraints for data validation (without IF NOT EXISTS which isn't supported)
DO $$
BEGIN
    -- Add Bitcoin address constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_bitcoin_address' 
    ...
    -- Add Lightning address constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_lightning_address' 
        AND table_name = 'profiles'
    ) THEN
        ALTER TABLE public.profiles
